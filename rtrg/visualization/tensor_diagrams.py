...
from ..core.tensors import LorentzTensor, IndexStructure
from ..core.fields import Field, FieldRegistry


def visualize_tensor_network(tensors: Dict[str, LorentzTensor], 
    ...
    for i, (name, tensor) in enumerate(tensors.items()):
        ...
        for j, idx_name in enumerate(tensor.indices.names):
            ...
            G.add_node(idx_node, pos=(idx_x, idx_y), 
    ...
    nx.draw_networkx_nodes(G, pos, nodelist=tensor_nodes, 
    ...
    nx.draw_networkx_nodes(G, pos, nodelist=covariant_nodes,
                          node_shape='o', node_size=200,
                          node_color='red', alpha=0.7, label='Covariant')
    
    nx.draw_networkx_nodes(G, pos, nodelist=contravariant_nodes,
                          node_shape='^', node_size=200, 
                          node_color='blue', alpha=0.7, label='Contravariant')
    ...
    nx.draw_networkx_edges(G, pos, edgelist=tensor_index_edges,
                          edge_color='gray', alpha=0.6, width=1)
    
    nx.draw_networkx_edges(G, pos, edgelist=contraction_edges,
                          edge_color='red', alpha=0.8, width=3,
                          style='dashed')
...
def visualize_index_structure(tensor: LorentzTensor, 
    ...
    tensor_box = patches.Rectangle((0.4, 0.4), 0.2, 0.2, 
    ...
    positions = []
    for i, (name, idx_type, symmetry) in enumerate(zip(indices.names, indices.types, indices.symmetries)):
        ...
        ax1.text(x + offset_x, y + offset_y, name, ha='center', va='center', 
        ...
        if symmetry != 'none':
            ...
            ax1.text(x, y - 0.03, symbol, ha='center', va='top', fontsize=8, 
    ...
    legend_elements = [
        plt.Line2D([0], [0], marker='^', color='w', markerfacecolor='blue', 
                  markersize=10, label='Contravariant (upper)'),
        plt.Line2D([0], [0], marker='v', color='w', markerfacecolor='red', 
                  markersize=10, label='Covariant (lower)'),
        plt.Line2D([0], [0], marker='o', color='green', markersize=8, 
                  label='S: Symmetric, A: Antisymmetric', linestyle='None')
    ]
    ...
    if tensor.rank <= 2:
        if tensor.rank == 0:
            # Scalar
            ax2.text(0.5, 0.5, f'Scalar value: {tensor.components:.3f}', 
                    ha='center', va='center', fontsize=12)
        elif tensor.rank == 1:
            # Vector
            components = np.real(tensor.components) if np.isreal(tensor.components).all() else np.abs(tensor.components)
        ...
        ax2.text(0.5, 0.5, f'Rank {tensor.rank} tensor\n(too high to visualize)', 
                ha='center', va='center', fontsize=12)
...
def plot_tensor_operation(operation_name: str,
    ...
    for name, tensor in input_tensors.items():
        ...
        ax_inputs.text(0.05, y_pos, info_text, transform=ax_inputs.transAxes, 
    ...
    ax_operation.text(0.05, 0.95, operation_text, transform=ax_operation.transAxes,
    ...
    ax_formula.text(0.5, 0.5, formula, transform=ax_formula.transAxes,
    ...
    ax_result.text(0.05, 0.95, result_text, transform=ax_result.transAxes,
                  fontsize=10, verticalalignment='top',
...
def plot_field_hierarchy(registry: FieldRegistry, 
    ...
    for node in physical_nodes:
        ...
        # Draw field box
        box = patches.Rectangle((x-0.08, y-0.05), 0.16, 0.1, 
                               facecolor=color, edgecolor='black', linewidth=2)
        ax.add_patch(box)
        
        # Field name and properties
        ax.text(x, y+0.02, node, ha='center', va='center', 
               fontsize=10, fontweight='bold')
        
        # Dimension info
        dim_text = f"[{data['dimension']:.1f}]"
        ax.text(x, y-0.02, dim_text, ha='center', va='center', 
        ...
        if properties:
            ...
            ax.text(x, y-0.04, prop_text, ha='center', va='center',
    ...
    for node in response_nodes:
        ...
        ax.text(x, y+0.02, display_name, ha='center', va='center',
               fontsize=10, fontweight='bold')
    ...
    for edge in G.edges(data=True):
        if edge[2]['relation'] == 'response':
            ...
            arrow = patches.FancyArrowPatch((x1+0.08, y1), (x2-0.08, y2),
    ...
    ax.text(0.2, 0.95, 'Physical Fields', ha='center', va='center',
           fontsize=14, fontweight='bold', transform=ax.transAxes,
           bbox=dict(boxstyle='round', facecolor='lightblue', alpha=0.7))
    
    ax.text(0.8, 0.95, 'Response Fields (MSRJD)', ha='center', va='center',
           fontsize=14, fontweight='bold', transform=ax.transAxes,
    ...
    legend_elements = [
        ...
        patches.Patch(facecolor='orange', edgecolor='black', 
                     linestyle='--', label='Response Fields'),
    ...
    ax.text(0.02, 0.15, prop_text, transform=ax.transAxes, fontsize=9,
           bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8))
    
    # Add dimension explanation
    dim_text = "Dimensions:\n[n] = Engineering dimension\nResponse: [physical] - 4"
    ax.text(0.02, 0.02, dim_text, transform=ax.transAxes, fontsize=9,
