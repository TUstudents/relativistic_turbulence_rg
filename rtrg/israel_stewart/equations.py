...
class IsraelStewartSystem:
    ...
    def __init__(self, parameters: IsraelStewartParameters, metric: Optional[Metric] = None):
        """
        ...
        self.rho = self.field_registry.get_field("rho")
        self.u = self.field_registry.get_field("u") 
        self.pi = self.field_registry.get_field("pi")
        self.Pi = self.field_registry.get_field("Pi")
        self.q = self.field_registry.get_field("q")
    ...
    def get_conservation_equations(self) -> Dict[str, sp.Expr]:
        ...
        for nu_val in range(4):
            energy_momentum_conservation = sp.Sum(
                sp.Derivative(T_munu[mu, nu_val], self.coordinates[mu]),
                (mu, 0, 3)
            )
        ...
        particle_conservation = sp.Sum(
            sp.Derivative(n_mu[mu], self.coordinates[mu]),
            (mu, 0, 3)
        )
