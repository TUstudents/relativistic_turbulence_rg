...
class Field(ABC):
    ...
    def __init__(self, properties: FieldProperties, metric: Metric | None = None):
        """Initialize field
    ...
    def create_tensor(self, components: np.ndarray) -> LorentzTensor:
        ...
        # Apply constraints
        if self.properties.is_symmetric and self.rank >= 2:
            tensor = self._make_symmetric(tensor)
    ...
    def _make_symmetric(self, tensor: LorentzTensor) -> LorentzTensor:
        """Make tensor symmetric by averaging over index permutations"""
        if tensor.rank != 2:
            return tensor  # Only implement for rank-2 tensors for now

        # For rank-2: T^{(μν)} = (1/2)(T^μν + T^νμ)
...
class FourVelocityField(Field):
    ...
    def is_normalized(self, components: np.ndarray) -> bool:
        ...
        return bool(abs(u_dot_u + c_squared) < 1e-12)
...
class ShearStressField(Field):
    ...
    def evolution_equation(self, tau_pi: float = 1.0, eta: float = 1.0, **kwargs: Any) -> sp.Expr:
        """
        ...
        tau_pi_sym = sp.Symbol("tau_pi")
        eta_sym = sp.Symbol("eta")

        # τ_π ∂_t π^{μν} + π^{μν} = 2η σ^{μν} - τ_π π^{μν} θ + ...
        evolution = (
            tau_pi_sym * sp.Derivative(self.symbol, t)
            + self.symbol
            - 2 * eta_sym * sigma(t)
            + tau_pi_sym * self.symbol * theta
        )
...
class BulkPressureField(Field):
    ...
    def evolution_equation(self, tau_Pi: float = 1.0, zeta: float = 1.0, **kwargs: Any) -> sp.Expr:
...
class FieldRegistry:
    ...
    def create_is_fields(self, metric: Metric | None = None) -> None:
        ...
        for field_obj in fields_to_create:
