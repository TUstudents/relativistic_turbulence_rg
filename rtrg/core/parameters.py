...
from .constants import PhysicalConstants, UnitSystem
...
class ISParameters:
    ...
    def _check_causality(self) -> bool:
        ...
        try:
            # Maximum characteristic speed from linearized analysis
            # This is a simplified check - full analysis would solve dispersion relation
            
            # Shear mode speed
            v_shear = np.sqrt(self.eta / (self.energy_density * self.tau_pi))
            
            # Bulk mode contributions
            v_bulk_contrib = np.sqrt(self.zeta / (self.energy_density * self.tau_Pi))
            
            # Combined maximum speed (approximate)
            v_max = np.sqrt(self.cs**2 + v_shear**2 + v_bulk_contrib**2)
            
            return v_max < PhysicalConstants.c
            
        except (ZeroDivisionError, ValueError):
            # If we can't compute, assume valid for now
            return True
    ...
    def to_dimensionless(self, L0: float, rho0: float, 
                        T0: Optional[float] = None) -> 'ISParameters':
    ...
    def dimensionless_numbers(self, L: Optional[float] = None, 
                             v: Optional[float] = None) -> Dict[str, float]:
    ...
    def kinetic_theory_relations(self, particle_mass: float = 1.0,
                               cross_section: float = 1.0) -> 'ISParameters':
    ...
    def update(self, **kwargs) -> 'ISParameters':
    ...
    def __str__(self) -> str:
        ...
        return (f"ISParameters(η={self.eta:.3g}, ζ={self.zeta:.3g}, "
