...
class TestISParameters:
    ...
    def test_causality_check(self, is_parameters):
        ...
        # Create parameters that violate causality
...
class TestParameterTransformations:
    ...
    def test_parameter_copy_and_update(self, is_parameters):
        ...
        # Test update (use parameters that won't violate causality)
        updated = is_parameters.update(eta=0.2, tau_pi=0.1)

        assert updated.eta == 0.2
        assert updated.tau_pi == 0.1
        assert updated.zeta == is_parameters.zeta  # Unchanged
        assert is_parameters.eta == 0.5  # Original unchanged
...
class TestKineticTheoryRelations:
    ...
    def test_kinetic_theory_consistency(self):
        ...
        params = ISParameters(
            ...
            zeta=0.05,
            kappa=0.2,
            tau_pi=0.1,  # Increased to avoid causality violation
            tau_Pi=0.1,
            tau_q=0.15,
            cs=1.0 / np.sqrt(3),
            temperature=1.0,
