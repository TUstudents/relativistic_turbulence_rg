...
### Mind Map (Code Overview)
- The file `mind-map.md` contains an auto-generated overview of classes and functions grouped by module.
- It is generated by `tools/generate_mind_map.py` and is automatically refreshed on commit via a pre-commit hook.
- Regenerate manually:
  - `uv run python tools/generate_mind_map.py` (add `--summaries` for brief docstrings)
  - The script only updates the section between special markers and leaves curated content intact.
